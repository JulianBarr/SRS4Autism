...
# Vowels that need feeding
BASIC_VOWELS = ['a', 'o', 'e', 'i', 'u', 'ü']

# Expanded Donor List (In order of preference)
# We will try to steal from Stage 1 first, then Stage 2, then Stage 3.
EXPANDED_DONORS = [
    'b', 'p', 'm', 'f',       # Stage 1
    'd', 't', 'n', 'l',       # Stage 2 (Good for 'e' and 'ü')
    'g', 'k', 'h'             # Stage 3 (Good for 'e')
]
...
# ==========================================
def run_dry_run():
    print(f"\n📊 PINYIN DISTRIBUTION V3 (Expanded Backfill)")
    ...
    # 3. "FEED THE VOWELS" LOGIC (Expanded)
    moved_log = []
    ...
    for vowel in BASIC_VOWELS:
        ...
        if current_count < 2:
            ...
            # Iterate through our EXPANDED donor list
            for init in EXPANDED_DONORS:
                ...
                for item in candidates:
                    ...
                    # Match check: does this syllable actually contain our target vowel?
                    # Special check for 'ü' because pinyin input might vary (v vs ü)
                    is_match = (final == vowel)
                    if vowel == 'ü' and final == 'v': is_match = True
