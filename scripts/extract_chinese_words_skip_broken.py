#!/usr/bin/env python3
"""
Extract Chinese words, skip broken blocks
==========================================
Process Chinese words section and skip any blocks with parse errors.
...
OUTPUT_VALID = PROJECT_ROOT / "knowledge_graph" / "world_model_chinese_words_valid.ttl"
OUTPUT_SKIPPED = PROJECT_ROOT / "knowledge_graph" / "world_model_chinese_words_skipped.ttl"

def main():
    print("🚀 Extracting Chinese words (skip broken blocks)...")
    ...
    # Find Chinese words section (lines starting with srs-kg:word-zh-)
    print("📍 Finding Chinese words section...")
    word_blocks = []
    current_block = []
    in_chinese_word = False

    for i, line in enumerate(lines):
        stripped = line.strip()

        # Detect start of Chinese word block
        if stripped.startswith('srs-kg:word-zh-'):
            if current_block:
                word_blocks.append(''.join(current_block))
            current_block = [line]
            in_chinese_word = True
        elif in_chinese_word:
    ...
    print(f"✅ Found {len(word_blocks):,} Chinese word blocks")
    ...
    with open(OUTPUT_SKIPPED, 'w', encoding='utf-8') as f:
        f.write("# SKIPPED CHINESE WORD BLOCKS\n")
