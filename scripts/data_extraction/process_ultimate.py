...
4. 提取共享属性：如果有适用于该大项目的注意事项，请置于 `####` 标题下，命名为 **Shared Precautions:**。
...
def process_document(input_file, pdf_file, output_file, checkpoint_file, offset):
    ...
    for chunk in chunks:
        if chunk["type"] == "gemini":
            ...
            # 在单次对话的 prompt 中也再次强调语义分配
            prompt = [
                *images,
                f"这是工作区文本，里面包含了你必须【原样保留】的目录结构标题。请仔细阅读图片，将图片底部的活动建议根据语义精准分配到对应的目标下方，并使用 ##### 5级标题结构。\n\n工作区文本：\n{chunk['text']}"
            ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process Workspace Markdown with Gemini Vision")
