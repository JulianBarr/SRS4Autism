...
def parse_vbmapp_chunk(text_chunk, chunk_index):
    ...
    prompt = """
    You are an expert ontologist and behavioral analyst. Your task is to extract a Directed Acyclic Graph (DAG) ontology from the provided text, which contains a section of the VB-MAPP scoring instructions.

    Extract the data into a flat JSON array of objects representing Nodes (Domains, Milestones, Tasks).

    ### JSON SCHEMA PER OBJECT:
    {
      "id": "A unique, lowercase identifier (e.g., 'domain-mand', 'mand-1-m', 'mand-1-a')",
      "type": "Domain" | "Milestone" | "Task",
      "domain": "The standardized Domain name (e.g., 'Mand', 'Tact')",
      "level": 1,
      "title": "The exact title or code (e.g., 'Mand 1-M', '1-a')",
...
if __name__ == "__main__":
    ...
    chunks = extract_text_from_pdf(PDF_FILE, 37, 55)
    ...
    for index, chunk in enumerate(chunks):
        ...
        checkpoint_file = f"vbmapp_checkpoint_{index + 1}.json"
        ...
        try:
            ...
            with open(f"vbmapp_error_{index + 1}.txt", 'w', encoding='utf-8') as f:
                f.write(json_output)
    ...
    with open("vbmapp_level1_master.json", 'w', encoding='utf-8') as f:
        json.dump(final_nodes, f, indent=4, ensure_ascii=False)
        
    print("Success! Master DAG Ontology saved to vbmapp_level1_master.json")
