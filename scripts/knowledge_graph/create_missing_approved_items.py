...
# -*- coding: utf-8 -*-
"""
Create missing approved items that should be in the database.
Based on:
- UI shows 105 approved
- CSV only has 55 approved
- ~35 items are missing from DB
- Focus on items with images (most likely to be approved)
"""
...
def create_missing_approved_items():
    """Create missing approved items"""
    print("=" * 80)
    print("Create Missing Approved Items")
    print("=" * 80)
    print()
    ...
    # Load all suggestions with images
    candidates = []
    if SUGGESTIONS_FILE.exists():
        with open(SUGGESTIONS_FILE, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                if syllable and word and word != 'NONE' and has_image:
                    candidates.append({
                        'syllable': syllable,
                        'word': word,
    ...
    print(f"📋 Found {len(candidates)} candidates with images")
    print()
    ...
    if not missing_items:
        print("✅ No missing items to create!")
        return
